...
function send2DTextureToGLSL(val, textureUnit, uniformName) {
  glTextureUnit = determineGLTextureUnit(textureUnit);
  uniformVar = getUniformLocation(gl.program, uniformName);
  gl.activeTexture(glTextureUnit);
  gl.bindTexture(gl.TEXTURE_2D, glTextureUnit);
...
function create2DTexture(imgPath, magParam, minParam, wrapSParam, wrapTParam, callback) {
  ...
  textImg.onload = function() {
    ...
    gl.texParameteri(gl.TEXTURE_2D, magParam);
    gl.texParameteri(gl.TEXTURE_2D, minParam);
    gl.texParameteri(gl.TEXTURE_2D, wrapSParam);
    gl.texParameteri(gl.TEXTURE_2D, wrapTParam);
...
function sendUniformMatToGLSL(val, uniformName) {
  u_ModelMatrix = gl.getUniformLocation(gl.program, uniformName);
  if (!u_ModelMatrix) {
    console.log('Failed to get storage location of u_ModelMatrix');
    return;
  }

  gl.uniformMatrix4fv(u_ModelMatrix, false, val);
...
function sendAttributeBufferToGLSL(data, dataCount, attribName) {
  ...
  a_Position = gl.getAttribLocation(gl.program, attribName);
  if (a_Position < 0) {
...
function sendUniformVec4ToGLSL(val, uniformName) {
  ...
  u_FragColor = gl.getUniformLocation(gl.program, uniformName);
  if (!u_FragColor) {
      console.log('Failed to get storage location of u_FragColor');
      return;
  }
  gl.uniform4f(u_FragColor, val[0], val[1], val[2], val[3]);
}
