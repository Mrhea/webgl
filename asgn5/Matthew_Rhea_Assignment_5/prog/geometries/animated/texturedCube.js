...
class MultiTextureCube extends TiltedCube {
  ...
  /*******************************
   * 
   * PERFORMNACE UPDATE IN PROGRESS
   * EXAMPLE OF OPTIMIZED RENDER FUNCTION FOR MULTITEXTURE CUBE
   * 
   ******************************/
  // render() {
  //   // Switch to proper shading program
  //   useShader(gl, this.shader);

  //   // Send the texture and modelMatrix of the MultiTextureCube
  //   send2DTextureToGLSL(this.textures[0], 0, 'u_Sampler');
  //   sendUniformMatToGLSL(this.modelMatrix.elements, 'u_ModelMatrix');

  //   sendAttributeDataToGLSL(this.dataArray, 'Textured', this.dataCounts, 
