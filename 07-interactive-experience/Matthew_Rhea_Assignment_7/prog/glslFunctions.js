...
function sendUniformMatToGLSL(val, uniformName) {
  let uniformMat = gl.getUniformLocation(gl.program, uniformName);
  if (!uniformMat) {
  ...
  switch (val.length) {
    case 16:
      gl.uniformMatrix4fv(uniformMat, false, val);
      break;
    case 9:
      gl.uniformMatrix3fv(uniformMat, false, val);
      break;
    case 4:
      gl.uniformMatrix2fv(uniformMat, false, val);
      break;
    default:
      console.log('ERROR: Incorrect val size sent to Matrix in GLSL');
      return  
  }
}
...
function sendUniformVec4ToGLSL(val, uniformName) {
  ...
  // console.log('SUCCEEDED in getting storage location of: ' + uniformName);
  switch (val.length) {
    case 2:
    ...
    case 4:
      ...
      break;
    default:
      console.log('ERROR: Incorrect val vector input size sent to GLSL');
      return
  }
}
...
/*****************************************************************************
 * 
 *           PERFORMANCE OPTIMIZING CODE (hopefully)
 * 
 ****************************************************************************/
...
function saveShaderLocations(gl, shaderName, shaderProgram, uniformList, attributeList) {
  ...
  for(var j = 0; j < attributeList.length; j++) {
    ...
    if (attributeLocations[shaderName][attributeList[j] < 0]) {
      console.log('Failed to get storage location of: ' + attributeList[i]);
      return -1;
    }
  }
}
...
function sendAttributeDataToGLSL(data, shaderName, dataCounts, attribNames) {
  ...
  gl.bufferData(gl.ARRAY_BUFFER, data, gl.STATIC_DRAW);  // Error here when loadedobj not rendered
